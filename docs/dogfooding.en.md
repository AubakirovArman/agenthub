...
`scripts/kimi-auth-check.sh` is the safer first step when the RC gate is blocked on Kimi. It tests both official Moonshot endpoints (`https://api.moonshot.ai/v1` and `https://api.moonshot.cn/v1`), writes redacted artifacts under `target/dogfood/kimi-auth/`, and writes `target/dogfood/kimi-auth-report.json`. If one endpoint passes, the report includes `passed_endpoint` and the next action preserves that region for provider dogfood. If both endpoints return `auth_failed`, replace or rotate the Kimi/Moonshot API key before running provider dogfood.

After receiving a Kimi/Moonshot replacement key, first run `providers inspect-key kimi --from-file <new-key-file>` and `providers rehearse-unblock kimi --from-file <new-key-file>` to check the candidate and replacement path offline without writing, network access, or secret output, then run `providers preflight-key kimi --from-file <new-key-file>` to test it live. When the configured endpoint is official, preflight checks both Moonshot global and China endpoints and prints the exact `MOONSHOT_BASE_URL=... providers rc-unblock` command for the passing region. `providers rc-unblock kimi --from-file <new-key-file>` now repeats that no-write preflight before installation, installs the key only after a passing candidate, and reuses the passing region endpoint for `agenthub providers test kimi`, live Kimi provider dogfood, `scripts/rc-evidence-collect.sh`, and `scripts/rc-dogfood-gate.sh --check`. Product-CLI runs write `target/dogfood/kimi-rc-operator-receipt.json` for both successful and blocked attempts; blocked receipts include `attempt.status`, `attempt.reason`, and redacted Kimi auth report fields. Readiness output exposes the receipt path plus the latest redacted attempt summary. The existing two-step path still works after separate rotation with `providers rotate-key kimi`. If the provider test still fails, the command runs `scripts/kimi-auth-check.sh` anyway as diagnostics so the redacted two-endpoint auth report is refreshed before it returns `blocked`.

Do not use Kimi Code CLI credential files as replacement keys. Those files contain short-lived OAuth `access_token`/`refresh_token` material for the CLI, not Moonshot OpenAI-compatible API keys; AgentHub rejects that JSON before writing `.kimi` or running provider tests.

The RC evidence collector reads `target/dogfood/kimi-auth-report.json`. A blocked report becomes an open critical `kimi-auth` blocker in `scripts/rc-dogfood-gate.sh --check`; a passed report records the `kimi_auth` check, but the full RC still requires passed Kimi provider dogfood.
