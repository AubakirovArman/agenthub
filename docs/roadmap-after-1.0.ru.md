...
These are the next concrete engineering steps from the current `0.4.121-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.104` added `agenthub readiness completion --json` as the aggregate final completion bundle, `0.4.105` added shell plan aliases for mode/cost/Ops host controls, `0.4.106` makes `/sessions` a first-class chat-session alias while keeping transaction history on `/transactions`, `/tx`, and `/history`, `0.4.107` splits lightweight provider selection from provider setup, `0.4.108` release-gates the daily shell UX aliases without Git/`.agent` bootstrap, `0.4.109` makes that shell UX gate visible in the readiness checklist, `0.4.110` promotes the same shell UX smoke into source-backed `shell_ux_aliases` RC evidence required by the default dogfood gate, `0.4.111` adds offline Kimi replacement-key unblock rehearsal evidence, `0.4.112` restores Windows CI parity for that release line, `0.4.113` adds named readiness gap summaries across completion/evidence/checklist surfaces, `0.4.114` hardens shell/TUI regression coverage with no-bootstrap TUI smoke plus exact renderer snapshots, `0.4.115` hardens long-session evidence with no-network memory context receipts, enriched perf profiles, and source-backed compaction/resume/rewind/cost checks, `0.4.116` adds per-check recovery commands inside named readiness gaps so Kimi/dogfood blockers are actionable from the gap view itself, `0.4.117` makes the `provider_kimi` recovery commands self-contained for product and script-level readiness audits, `0.4.118` adds a Kimi RC operator receipt for successful unblock/dogfood/RC refresh attempts, `0.4.119` adds package-version metadata to readiness action/blocker snapshots, `0.4.120` writes operator receipts for blocked Kimi RC attempts too, and `0.4.121` exposes the latest Kimi RC operator receipt summary directly in readiness outputs.
...
| `0.4.75` | Ecosystem surface readiness coverage | Done: `agenthub readiness audit [--json]` now includes `ecosystem_surfaces`, verifying that the post-1.0 roadmap surface is exposed by the product CLI rather than only checking that roadmap files exist |
| `0.4.76` | Source-backed open blocker IDs | Done: `agenthub readiness audit`, `agenthub readiness blockers`, and `scripts/api-native-completion-audit.sh` now include open RC blocker IDs in the `open_blockers` detail, so Kimi/auth blockers are visible without reading raw evidence JSONL |
| `0.4.77` | Recovery-to-blockers automation path | Done: `agenthub providers recovery [--json]` now includes `agenthub readiness blockers --json --check` in provider and gate next commands before the full readiness audit |
| `0.4.78` | Per-blocker recovery commands | Done: `agenthub readiness blockers --json` now includes per-blocker `next_commands`, and the text report renders matching `blocker_next` lines |
| `0.4.79` | Per-check audit recovery commands | Done: `agenthub readiness audit --json` now includes per-check `next_commands` for incomplete rows, the text audit renders `check_next` lines, and the blocker view reuses the same commands |
| `0.4.80` | Public provider-route docs cleanup | Done: README, how-it-works, provider-route, and dogfooding docs now describe DeepSeek/Kimi as user-facing API providers and `command` as the internal deterministic runner |
| `0.4.81` | Provider recovery noise reduction | Done: `agenthub providers recovery [--json]` now marks already available providers as `ready` with no redundant provider-test command, keeping next steps focused on Kimi/readiness blockers |
| `0.4.82` | Clean readiness JSON during evidence refresh | Done: `agenthub readiness audit --json --check` and `agenthub readiness blockers --json --check` now capture noisy evidence-collector stdout/stderr so automation receives valid JSON even when the gate remains blocked |
| `0.4.83` | Completion-audit blocker-first recovery | Done: `scripts/api-native-completion-audit.sh --json --check` now lists `agenthub readiness blockers --json --check` before the full readiness audit, matching provider recovery and the roadmap's short-blocker-first workflow |
| `0.4.84` | Offline Kimi key inspection | Done: `agenthub providers inspect-key kimi [--from-file <new-key-file>]` safely reports credential fingerprint/shape, refuses known matching Kimi auth blockers, and threads the inspection step through provider recovery, readiness blockers, and script unblock paths before live preflight |
| `0.4.85` | Inspect-key Windows CI parity | Done: the Kimi inspect-key auth report fixture is serialized as JSON, so Windows path separators cannot invalidate the blocker evidence test |
| `0.4.86` | External blocker classification | Done: readiness audit and focused blocker reports now carry `blocker_kind` metadata, classifying Kimi auth as an external credential blocker while keeping recovery commands attached |
| `0.4.87` | Provider recovery CI determinism | Done: provider recovery text tests now clear provider credential environment, so tag CI cannot inherit external provider state and change the expected gate status |
| `0.4.88` | Provider recovery blocker classification | Done: provider recovery JSON/text now carries `blocker_kind`, making Kimi external credential blockers visible before the focused readiness blocker view |
| `0.4.89` | Readiness blocker scope summary | Done: readiness audit and focused blocker reports now carry top-level `blocker_scope` and `blocker_kinds`, classifying the current Kimi-gated state as `external_only` instead of ambiguous local incompleteness |
| `0.4.90` | Provider recovery scope summary | Done: provider recovery JSON/text now carries top-level `blocker_scope` and `blocker_kinds`, so the first recovery surface also distinguishes external-only provider blockers from local implementation gaps |
| `0.4.91` | Kimi inspect-key JSON | Done: `agenthub providers inspect-key kimi --json` now emits a redacted machine-readable offline credential report with source, fingerprint, shape classification, policy, status, failure flag, and next commands |
| `0.4.92` | Script completion-audit scope metadata | Done: `scripts/api-native-completion-audit.sh --json` now emits top-level `blocker_scope`/`blocker_kinds` plus per-check `blocker_kind` and `next_commands`, matching the product readiness audit external-only classification |
| `0.4.93` | Script completion-audit text scope | Done: text output from `scripts/api-native-completion-audit.sh` now prints top-level `blocker_scope` and `blocker_kinds`, matching JSON and product readiness text |
| `0.4.94` | Script completion-audit blocked check IDs | Done: JSON/text output from `scripts/api-native-completion-audit.sh` now prints top-level `blocked_checks`, listing the unresolved completion-audit row IDs directly |
| `0.4.95` | Product readiness blocked check IDs | Done: `agenthub readiness audit` and `agenthub readiness blockers` now print top-level `blocked_checks` in JSON/text output, matching the script completion-audit surface |
| `0.4.96` | Provider recovery blocked check IDs | Done: `agenthub providers recovery` now prints top-level `blocked_checks` in JSON/text output for blocked provider IDs and the dependent completion gate |
| `0.4.97` | Provider status blocker kinds | Done: raw `agenthub providers status --json` rows now carry `blocker_kind` for missing or blocked DeepSeek/Kimi providers, classifying auth/setup blockers as `external_credential` |
| `0.4.98` | Provider status Kimi auth evidence | Done: raw `agenthub providers status --json` Kimi rows now carry redacted `auth_status`, `auth_key_sha256_12`, `auth_key_source`, `credential_warning`, and `next_action` when the current key matches a blocked auth report |
| `0.4.99` | Provider status recovery hints | Done: raw `agenthub providers status --json` rows now carry readiness `check_id`, and blocked/missing rows carry per-row `next_commands` so automation can move from raw status to recovery without parsing `detail` |
| `0.4.100` | Readiness prompt-to-artifact checklist | Done: `agenthub readiness checklist [--json] [--check] [--no-refresh]` groups API-native 1.0 requirements into concrete files, commands, evidence sources, mapped readiness checks, blocker kinds, and recovery commands |
| `0.4.101` | RC evidence status CLI | Done: `agenthub readiness evidence [--json] [--check] [--no-refresh]` reports dogfood history, session/Ops/project/cost thresholds, provider evidence, RC checks, Kimi auth state, open blockers, and the final dogfood gate |
| `0.4.102` | Provider status credential classification | Done: `agenthub providers status --json` adds redacted `credential_classification` for Kimi credential blockers and blocks Kimi Code CLI OAuth JSON before a live auth report exists |
| `0.4.103` | Readiness next action plan | Done: `agenthub readiness next [--json] [--check] [--no-refresh]` reports the current phase, focus, stop reason, next milestone, immediate commands, verification commands, and deferred post-1.0 tracks from the same source-backed readiness audit |
| `0.4.104` | Readiness completion bundle | Done: `agenthub readiness completion [--json] [--check] [--no-refresh]` bundles final completion status, current action, prompt-to-artifact requirements, focused evidence, provider status rows, source files, blocker scope, blocked checks, and verification commands |
| `0.4.105` | Shell plan UI aliases | Done: `/mode chat|devops|project`, `/cost`, `/balance`, `/hosts`, and `/connect <host>` are parsed, shown in shell help/palette, documented, and wired to transcript mode overrides, local cost usage, and Ops host profiles |
| `0.4.106` | Chat session alias parity | Done: `/sessions` now lists/filter chat sessions like `/chats`, while `/transactions`, `/tx`, and `/history` remain the transaction-history surface |
| `0.4.107` | Provider selection clarity | Done: `/provider <id>` and `agenthub providers select <id>` now choose a ready DeepSeek/Kimi provider without entering setup, and blocked/missing providers print redacted recovery commands without changing config |
| `0.4.108` | Shell UX alias smoke gate | Done: `scripts/test-shell-ux-aliases.sh` now drives `/mode`, `/sessions`, `/cost`, `/balance`, `/hosts`, `/connect`, `/provider deepseek`, and `!command` through a no-Git/no-`.agent` shell session, and release readiness runs it |
| `0.4.109` | Shell UX checklist evidence | Done: `agenthub readiness checklist --json` now lists `scripts/test-shell-ux-aliases.sh` under `chat_ops_project_modes`, and tests cover that prompt-to-artifact mapping |
| `0.4.110` | Shell UX RC evidence | Done: dogfood reports now record `shell_ux_status` and `shell_ux_artifact`, the RC evidence collector emits `shell_ux_aliases`, and the default RC gate/readiness audits require `rc_check_shell_ux_aliases` |
| `0.4.111` | Kimi replacement-key unblock rehearsal | Done: `agenthub providers rehearse-unblock kimi --from-file <new-key-file>` inspects current/candidate credentials offline, prints only redacted fingerprints/shape, writes no key, opens no network, and feeds `kimi_unblock_rehearsal` RC evidence through dogfood |
| `0.4.112` | Windows CI parity for Kimi rehearsal | Done: the Unix-only test helper import is gated correctly, keeping the Kimi replacement-key rehearsal release line green on Windows CI |
| `0.4.113` | RC evidence dashboard/status polish | Done: `readiness completion`, `readiness evidence`, and `readiness checklist` print named `gap` summaries for shell UX, Kimi, dogfood, latency, and approval gaps without requiring raw JSONL inspection |
| `0.4.114` | Daily shell/TUI regression hardening | Done: the no-bootstrap shell UX smoke now verifies TUI status line, composer, event rail, live tool cards, provider panel, and next actions after shell shortcuts; renderer tests add an exact shell/TUI text snapshot for event rail and cost/status panels |
| `0.4.115` | Long-session and compaction dogfood | Done: `agenthub memory context` writes no-network compaction receipts, perf profiles record cost/context/resume/rewind evidence, and the release smoke proves enriched perf reports feed `rc_check_long_session_latency` |
| `0.4.116` | Named gap recovery commands | Done: named readiness gaps now carry per-check `next_commands` and text `gap_check_next` rows so Kimi/auth/dogfood recovery commands are visible beside the exact unresolved check |
| `0.4.117` | Kimi provider recovery parity | Done: `provider_kimi.next_commands` now starts with current key inspection and includes candidate inspection, rehearsal, preflight, RC unblock, provider test, Kimi auth check, and live Kimi dogfood in both product and script readiness surfaces |
| `0.4.118` | Kimi RC operator receipt | Done: successful `providers rc-unblock kimi` attempts now write `target/dogfood/kimi-rc-operator-receipt.json` and print provider/model/endpoint/token-cost/dogfood/readiness blocker rows for operator review |
| `0.4.119` | Readiness snapshot package metadata | Done: `readiness next` and `readiness blockers` JSON/text snapshots include `package_version`, matching `readiness completion` for release automation |
| `0.4.120` | Kimi blocked-attempt RC receipt | Done: blocked `providers rc-unblock kimi` attempts write the same operator receipt with `attempt.status`, `attempt.reason`, and safe Kimi auth report fields |
| `0.4.121` | Readiness-visible Kimi RC receipt | Done: readiness audit/next/blockers/checklist/evidence/completion JSON carries the receipt path plus redacted `latest_kimi_rc_attempt`, and text output prints matching attempt rows |
...
This is why the `v0.4.8` through `v0.4.121` bridge releases focus on global Chat/Ops memory, a review-gated memory inbox, budgeted memory-aware chat context, provider diagnostics, visible mode routing, explainable tool permissions, lazy project bootstrap, context compaction receipts, event-backed TUI visibility, visible transaction approval receipts, CI-friendly headless approval receipts, recoverable session reads, native DeepSeek/Kimi command-plan tool-call receipts, dashboard observability, API-native tool-result reinjection, tool registry policy hardening, review-only automatic memory extraction, terminal live tool cards, grouped/ranked memory inbox review, Ops host profiles/runbook receipts, 1.0 RC evidence collection, 1.0 RC evidence gating, safer Kimi auth-blocker diagnostics, headless Ops execution, RC acceptance rehearsal, richer source-backed evidence harvesting, dogfood report RC evidence summaries, archived acceptance rehearsal evidence, clearer Kimi endpoint/auth failure receipts, a Kimi unblock runbook, source-backed Kimi auth blocker evidence, 1.0 preparation Kimi gating, API-native RC evidence purity, Kimi auth doctor visibility, Windows CI Ops stability, Windows tag CI smoke stability, provider-test exit-code hardening, provider-test exit-code release smoke, Kimi blocked status visibility, Kimi auth retest from blocked status, provider unblock CLI runbooks, API-native completion audits, safe Kimi key rotation, first-class Kimi key rotation CLI UX, one-shot Kimi RC unblock orchestration, product CLI RC unblock UX, auth-failure diagnostic parity before returning blocked, Windows CI parity for script-backed RC auth diagnostics, one-shot product CLI Kimi key rotation plus RC unblock, no-write Kimi replacement key preflight, Kimi preflight endpoint sweep, RC unblock endpoint carry-forward, script Kimi auth endpoint carry-forward, Kimi CLI credential rejection, visible Kimi unblock warnings, Kimi auth/audit credential warnings, Kimi status/doctor credential warnings, Kimi auth source visibility, Kimi audit source visibility, an explicit MCP/A2A ecosystem planning surface, redacted provider status JSON, provider recovery reports, machine-readable completion-audit JSON, first-class product readiness audits, readiness audit Windows CI parity, focused readiness blocker reporting, a full post-1.0 ecosystem roadmap surface, readiness coverage for that surface, source-backed open blocker IDs, recovery-to-blockers automation, per-blocker recovery commands, per-check audit recovery commands, public provider-route docs cleanup, provider recovery noise reduction, clean machine-readable readiness JSON during evidence refresh, script-level blocker-first recovery, offline Kimi key inspection, inspect-key Windows CI parity, external credential blocker classification, deterministic provider recovery CI coverage, provider recovery blocker classification, top-level readiness blocker scope classification, provider recovery scope classification, machine-readable inspect-key reports, script-level completion-audit blocker scope metadata and blocked-check IDs, product readiness blocked-check IDs, provider recovery blocked-check IDs, provider status blocker kinds, provider status Kimi auth evidence fields, provider status recovery hints, provider status Kimi credential classification, prompt-to-artifact readiness checklists, focused RC evidence status reports, prioritized readiness-next action plans, aggregate readiness-completion bundles in JSON, shell plan aliases for mode/cost/Ops host controls, `/sessions` chat alias parity, safe `/provider` selection before setup, release-gated shell UX alias smoke coverage, shell UX checklist evidence, source-backed shell UX RC evidence, source-backed Kimi unblock rehearsal evidence, Windows CI parity for that rehearsal, named readiness gap summaries, daily shell/TUI regression hardening, long-session compaction evidence, per-check named gap recovery commands, Kimi provider recovery parity, Kimi RC operator receipts, readiness snapshot package metadata, blocked-attempt RC receipts, and readiness-visible Kimi RC attempt summaries rather than enabling MCP/A2A runtime early.

## Next Implementation Sequence

### Next Release Slices

| Slice | Focus | Exit Criteria |
|---|---|---|
| `0.4.114` | Daily shell/TUI regression hardening | Done: shell shortcuts, `/provider`, `!command`, event rail, live tool cards, provider panel, and cost/status surfaces are covered by no-bootstrap smoke and exact renderer snapshot checks |
| `0.4.115` | Long-session and compaction dogfood | Done: `agenthub memory context`, enriched perf profiles, `scripts/test-long-session-compaction.sh`, and stricter RC collection prove compaction, resume/rewind, memory context, and cost receipts before counting long-session evidence |
| `0.4.116` | Named gap recovery commands | Done: `readiness completion/evidence/checklist` JSON now includes per-check recovery commands inside named gaps, and text output prints `gap_check_next` for direct blocker triage |
| `0.4.117` | Kimi provider recovery parity | Done: `provider_kimi` recovery now includes current key inspection plus full replacement-key, provider-test, auth-check, and live dogfood commands in product and script audits |
| `0.4.118` | Kimi RC operator receipt | Done: successful `providers rc-unblock kimi` runs now write a JSON operator receipt plus text rows for provider, model, endpoint, token/cost observation, dogfood run id, readiness completion status, and remaining blockers |
| `0.4.119` | Readiness snapshot package metadata | Done: `readiness next` and `readiness blockers` JSON/text snapshots now include `package_version`, matching `readiness completion` for release automation |
| `0.4.120` | Kimi blocked-attempt RC receipt | Done: blocked `providers rc-unblock kimi` attempts now write the same operator receipt with `attempt.status`, `attempt.reason`, and safe Kimi auth report fields |
| `0.4.121` | Readiness-visible Kimi RC receipt | Done: readiness outputs now expose `kimi_rc_operator_receipt` and redacted `latest_kimi_rc_attempt` fields so the last blocked/successful RC attempt is visible from audit surfaces |
| `0.4.122` | Kimi unblock and RC refresh | Run the replacement-key path when a valid Moonshot key is available, refresh live Kimi provider dogfood, collect RC evidence, and keep readiness blocked only on genuinely unresolved dogfood items |
| `1.0 RC` | Real dogfood sign-off | 100+ real sessions, 20+ Ops flows, 20+ project-edit flows, DeepSeek/Kimi provider evidence, zero open blockers, and all required RC checks passed |

1. Keep `scripts/test-shell-ux-aliases.sh` green after every shell-facing change; it now feeds `shell_ux_aliases` RC evidence through dogfood reports and is part of the default readiness gate.
2. Run `AGENTHUB_DOGFOOD_ACCEPTANCE=1 scripts/dogfood.sh` or `scripts/rc-acceptance.sh` as a local rehearsal after risky runtime changes; it proves stats, Ops no-bootstrap, approval UX, resume and undo/rewind still produce inspectable artifacts.
3. After receiving a Kimi/Moonshot replacement key, first run `providers inspect-key kimi --json --from-file <new-key-file>` and `providers rehearse-unblock kimi --from-file <new-key-file>` to verify the credential shape and replacement-key path offline without writing, network access, or secret output, then run `providers preflight-key kimi --from-file <new-key-file>` to test the candidate live; on official endpoints it tests both Moonshot regions and prints a `MOONSHOT_BASE_URL=... providers rc-unblock` command when the replacement belongs to a specific region. Do not pass Kimi Code CLI credential JSON; AgentHub rejects that OAuth material before any write/provider test and now flags matching source-backed auth blockers through `providers inspect-key kimi`. `providers rc-unblock kimi --from-file <new-key-file>` also repeats no-write preflight before installation and carries the passing endpoint into `providers test kimi`, live Kimi provider dogfood, RC evidence collection, and the RC gate in the correct order.
4. Run `agenthub readiness completion --json --check` first when automation needs the single aggregate "done/blocked" bundle, then `agenthub readiness next --json --check` for the prioritized current action, then `agenthub readiness blockers --json --check` for the short machine-readable blocker list, then `agenthub readiness checklist --json --check` for the prompt-to-artifact requirement map, then `agenthub readiness evidence --json --check` for the focused dogfood/evidence counters, then `agenthub readiness audit --json --check` for the full gate. These commands refresh RC evidence without contaminating stdout and expose top-level `blocked_checks`, so CI can parse JSON even while the command exits non-zero for Kimi/RC blockers. For human review use `agenthub readiness completion --check`, `agenthub readiness next --check`, `agenthub readiness evidence --check`, and `agenthub readiness audit --check`; inspect `blocked_checks`, per-check `next_commands`/`check_next` rows, inspect `target/dogfood/rc-evidence.jsonl`, fill any remaining real daily evidence, and pass `scripts/rc-dogfood-gate.sh --check` without lowering thresholds. `scripts/api-native-completion-audit.sh --json --check` remains a compatible script-level evidence path with matching blocked-check IDs.
...
6. Stabilize release blockers found by dogfooding: Kimi auth/key setup, long-session latency, Ops receipts, resume/rewind, approval UX, and shell UX regression coverage.
7. Use `agenthub providers status --json` for raw provider state, `agenthub providers recovery --json` for redacted recovery automation, `agenthub readiness completion --json` for the aggregate completion bundle, `agenthub readiness next --json` for the current action plan, and `agenthub readiness audit --json` for final readiness gates without parsing human-readable status text.
8. Use `agenthub ecosystem status --json` as the machine-readable post-1.0 planning surface while 1.0 remains blocked.
9. Post-1.0: start MCP stdio client runtime only after Chat/Ops/Project, memory review, TUI visibility, and Ops host safety are stable in daily use.
