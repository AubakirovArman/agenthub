...
| `1.0 RC` | Real dogfooding gate | Collect and pass the `0.4.70` evidence gate with real daily usage, including stable resume/rewind/stats, 20+ Ops and 20+ project-edit flows, passed DeepSeek/Kimi provider dogfood, and no Kimi/auth/latency/approval blockers |
...
3. Run `scripts/api-native-completion-audit.sh --json --check` for automation and `scripts/api-native-completion-audit.sh --check` for human review, inspect the generated `target/dogfood/rc-evidence.jsonl`, fill any remaining real daily evidence, and pass `scripts/rc-dogfood-gate.sh --check` without lowering thresholds.
...
6. Use `agenthub providers status --json` for raw provider state, `agenthub providers recovery --json` for redacted recovery automation, and `scripts/api-native-completion-audit.sh --json` for final readiness gates without parsing human-readable status text.
7. Use `agenthub ecosystem status --json` as the machine-readable MCP/A2A planning surface while 1.0 remains blocked.
