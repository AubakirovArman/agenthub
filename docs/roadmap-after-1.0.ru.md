...
These are the next concrete engineering steps from the current `0.4.104-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.67` added a visible, disabled-by-default ecosystem planning surface, `0.4.68` adds machine-readable provider status, `0.4.69` turns that status into a redacted provider recovery report for DeepSeek/Kimi automation, `0.4.70` makes the final completion audit machine-readable for release gates, `0.4.71` exposes that gate as first-class product CLI, `0.4.72` fixes Windows CI parity for that readiness surface, `0.4.73` adds a focused readiness blocker view for release stabilization, `0.4.74` expands the post-1.0 ecosystem surface to all strategic tracks from the external roadmap, `0.4.75` makes that surface part of the product readiness audit, `0.4.76` makes open RC blocker IDs visible in the readiness blocker detail, `0.4.77` wires provider recovery directly to the focused readiness blocker surface, `0.4.78` adds per-blocker recovery commands to that surface, `0.4.79` makes the full readiness audit itself carry per-check recovery commands, `0.4.80` clarifies public docs so compatibility `agent.adapter` language does not read like external CLI provider support, `0.4.81` keeps provider recovery next commands focused on active blockers instead of retesting already ready providers, `0.4.82` keeps readiness JSON parseable when the evidence collector refreshes data, `0.4.83` aligns the script-level completion audit with the focused blocker-first recovery path, `0.4.84` adds an offline Kimi key inspection step before any live preflight/rotation, `0.4.85` restores Windows CI parity for that inspect-key coverage, `0.4.86` classifies external Kimi credential blockers in readiness JSON/text output, `0.4.87` isolates provider recovery test state so tag CI remains deterministic, `0.4.88` carries blocker_kind into provider recovery JSON/text, `0.4.89` adds top-level readiness blocker scope metadata so automation can tell the remaining gate is external-only, `0.4.90` carries the same scope summary into provider recovery, `0.4.91` makes Kimi inspect-key machine-readable, `0.4.92` gives the script-level completion audit the same blocker scope and per-check recovery metadata, `0.4.93` mirrors that top-level scope in text completion-audit output, `0.4.94` adds top-level `blocked_checks` to script-level completion-audit JSON/text output, `0.4.95` adds the same top-level `blocked_checks` summary to product readiness audit/blocker JSON and text, `0.4.96` adds provider recovery `blocked_checks` so the first recovery surface lists blocked provider/gate IDs directly, `0.4.97` adds per-provider `blocker_kind` to raw `providers status --json`, `0.4.98` adds structured redacted Kimi auth evidence fields to that same raw provider status JSON, `0.4.99` adds provider `check_id` plus per-row recovery commands to the raw status JSON, `0.4.100` adds `agenthub readiness checklist --json` as a prompt-to-artifact requirement map, `0.4.101` adds `agenthub readiness evidence --json` as a focused RC evidence status report, `0.4.102` adds redacted Kimi `credential_classification` to raw provider status JSON while blocking Kimi Code CLI OAuth JSON before any live auth report exists, `0.4.103` adds `agenthub readiness next --json` as a single prioritized current-action surface, and `0.4.104` adds `agenthub readiness completion --json` as the aggregate final completion bundle over readiness-next/checklist/evidence/provider status.
