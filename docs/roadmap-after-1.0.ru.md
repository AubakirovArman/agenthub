...
These are the next concrete engineering steps from the current `0.4.68-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.67` added a visible, disabled-by-default ecosystem planning surface, and `0.4.68` adds machine-readable provider status for DeepSeek/Kimi recovery automation.
...
| `1.0 RC` | Real dogfooding gate | Collect and pass the `0.4.68` evidence gate with real daily usage, including stable resume/rewind/stats, 20+ Ops and 20+ project-edit flows, passed DeepSeek/Kimi provider dogfood, and no Kimi/auth/latency/approval blockers |
...
6. Use `agenthub providers status --json` to feed provider recovery automation without parsing human-readable status text.
