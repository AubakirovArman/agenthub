...
DeepSeek/Kimi project execution can first run bounded AgentHub-owned `read_file`, `list_dir`, `search`, and read-only `shell` tools, records redacted `tool_results_<role>.json` receipts, reinjects those results into the same provider turn, then requests a native `agenthub_command_plan` tool call or JSON fallback and permission-checks proposed commands before execution.
