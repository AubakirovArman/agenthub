...
AgentHub is a local transactional runtime for AI coding agents. It wraps API-native DeepSeek/Kimi provider work and deterministic command execution with isolated worktrees, verifier checks, rollback, memory, reports, and dashboards.
...
- Plain `agenthub` starts chat mode without requiring Git or `.agent`; project transactions initialize those only when needed.
...
- Check Kimi API directly with `KIMI_API_KEY=... agenthub providers test kimi`.
- Use `/chats`, `/search`, `/rename`, `/pin`, and `/unpin` to manage chat sessions with auto titles; filter with `/chats status:COMMITTED provider:deepseek date:today`.
