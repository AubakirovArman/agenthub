...
`deepseek` and `kimi` support OpenAI-compatible HTTP and HTTPS endpoints with bearer-token auth, timeouts, structured provider-test receipts, streaming chat, budgeted memory-aware chat context, and API-native project command execution. Shell actions уже имеют explainable tool permission profiles, но broader structured tool loop, full-screen TUI и automatic memory extraction всё ещё в работе.
