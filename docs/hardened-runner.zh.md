...
`runner.json` 记录 timeout、CPU、memory、disk、network 和 filesystem policy。当前 local execution 实际执行 timeout 和 process-tree cleanup。Docker remote runners 会在设置 `AGENTHUB_CPU_CORES`、`AGENTHUB_MEMORY_MB` 和 `AGENTHUB_NETWORK_MODE` 时应用 CPU、memory 和 network options。Disk 仍作为未来 hardened backends 的 recorded policy slot。
