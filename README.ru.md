...
AgentHub — локальный транзакционный runtime для AI coding agents. Его user-facing LLM provider surface — API-native DeepSeek и Kimi; AgentHub оборачивает provider work в isolated worktrees, command policy, verifier checks, rollback, memory, reports и dashboards.
...
- `/chats`, `/search`, `/rename`, `/pin` и `/unpin` управляют chat sessions прямо внутри shell; `/chats status:COMMITTED provider:deepseek date:today` фильтрует sessions.
...
`agenthub serve` держит local dashboard обновлённым: provider status, role/fallback setup, pending approvals, recent memory facts, transaction history, context receipts, chat/provider events, session recovery events, tool-loop receipts, tool logs и panes для report/diff/log.

## DeepSeek и Kimi API

AgentHub v0.4 переходит на API-native провайдеры. Настрой DeepSeek или Kimi через environment variables, после чего chat и project tasks идут через логирование и память AgentHub:
...
Kimi настраивается тем же способом:
...
- DeepSeek и Kimi используют API requests на стороне AgentHub и API keys из environment.
- Streaming chat и API-native project tool execution ещё подключаются.
