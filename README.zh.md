...
详细文档: [How it works](docs/how-it-works.en.md), [PRD tracker](docs/prd-tracker.zh.md), [PRD audit](docs/prd-audit.zh.md), [TUI](docs/tui.zh.md), [Web Dashboard](docs/web-dashboard.zh.md), [AAL](docs/aal.zh.md), [Workspaces](docs/workspaces.zh.md), [MediaWorkspace](docs/media-workspace.zh.md), [Research](docs/research-profile.zh.md), [IDE](docs/ide.zh.md), [Natural language](docs/natural-language.zh.md), [Topologies](docs/topologies.zh.md), [Agent adapters](docs/agent-adapters.zh.md), [Runtime and repair](docs/runtime-repair.zh.md), [Context maps](docs/context-maps.zh.md), [LLM Gateway](docs/llm-gateway.zh.md), [Plugin ecosystem](docs/plugin-ecosystem.zh.md), [Enterprise](docs/enterprise.zh.md), [Русский](docs/how-it-works.ru.md), [中文](docs/how-it-works.zh.md), [Қазақша](docs/how-it-works.kk.md)
...
- 事务型执行内核；
- 基于 git worktree 隔离的 `CodeWorkspace`、`ContentWorkspace`、`DataWorkspace`、`InfraWorkspace`、`MediaWorkspace`、`ResearchWorkspace`；
- journal、report、DAG、AgentIR、context pack 和 verifier logs；
- diff guard、sync check、rollback 和成功后 commit；
- verifier commands、runtime smoke checks，以及 content/data/infra 的 domain verifiers；
...
- plugin package scaffold、manifest validation、trust model 和 lock files；
- agent adapter routing、CLI dry-run invocation、prompts 和 transcripts；
- planner/executor、generator/critic、reviewer/repair、swarm research 和 manager/worker DAGs 的 multi-role topologies；
...
- standalone AAL parser，带 diagnostics 和 AgentSpec YAML output；
- terminal TUI dashboard，用于查看 transactions、DAG、verifier、cost、memory 和 approvals；
- static browser dashboard，用于查看 transactions、timeline、agent trace、memory graph、skills、policies、costs 和 reports；
- VS Code extension，用于查看 transaction、memory、AgentSpec、approval 和 DAG；
- enterprise policy source、RBAC checks、secret checks、runner/model routing、audit log 和 compliance reports。
...
AAL 支持 `workspace`、`goal`、`use skill`、`allow`、`deny`、`rules`、`execute`、`verify`、runtime smoke routes 和 transaction policy，然后输出 AgentSpec YAML。参见 [AAL](docs/aal.zh.md)。
...
Dashboard 会写入 `index.html`、`data.json`、`data.js`、`dashboard.css` 和 `dashboard.js`。它展示 recent transactions、journal timeline、DAG roles、memory graph、skills、enterprise policy summary、cost analytics 和 report links。参见 [Web Dashboard](docs/web-dashboard.zh.md)。
...
`inspect` 会验证 semver package versions、safe relative paths、referenced skill manifests 和 workspace schemas。已安装 plugin lock 存在 `.agent/plugins/installed.json`；已安装 skill 版本锁定在 `.agent/skills/installed.json`。

## Enterprise

Phase 14 提供 enterprise governance。Policy 位于 `.agent/enterprise/policy.yaml`，也可以来自 central `AGENTHUB_POLICY_PATH`；secret checks 不打印值；runner 与 private model routing 由 policy 控制；audit events 和 compliance reports 位于 `.agent/enterprise/`。
