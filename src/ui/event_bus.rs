    ...
    pub fn from_chat(row: ChatEventRow) -> Option<Self> {
        ...
        Some(Self {
            ...
            data: json!({
                ...
                "max_prompt_tokens": row.event.max_prompt_tokens,
                "memory_records_available": row.event.memory_records_available,
                "memory_records_expired": row.event.memory_records_expired,
                "memory_records_conflict_suppressed": row.event.memory_records_conflict_suppressed,
                "memory_records_budget_dropped": row.event.memory_records_budget_dropped,
                "memory_tokens": row.event.memory_tokens,
                "max_memory_tokens": row.event.max_memory_tokens,
                "recent_messages": row.event.recent_messages,
                "recent_messages_dropped": row.event.recent_messages_dropped,
                "context_compressed": row.event.context_compressed,
                "pending_memory_included": row.event.pending_memory_included,
                "completion_tokens": row.event.completion_tokens,
                "total_tokens": row.event.total_tokens,
                "estimated_input_cost_usd": row.event.estimated_input_cost_usd,
                "estimated_output_cost_usd": row.event.estimated_output_cost_usd,
                "estimated_cost_usd": row.event.estimated_cost_usd,
                "pricing_source": row.event.pricing_source,
